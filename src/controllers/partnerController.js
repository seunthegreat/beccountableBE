...
const getMembersUserReports = async (req, res) => {
    const { memberId } = req.params;

    try {
      // Find all partner requests where sender.memberId matches the memberId parameter
      const partners = await Partner.find({ 'recipient.memberId': memberId });

      // Get the lastSeen for each user and add it to the response
      const partnerWithUpdatedLastSeen = await Promise.all(partners.map(async (request) => {
        const recipientUser = await User.findOne({ memberId: request.sender.memberId });
      ...
      res.status(200).json({
        success: true, 
        message: "List of all members user reports",
        data: partnerWithUpdatedLastSeen
      });
    } catch (error) {
      // Send an error response if something goes wrong
